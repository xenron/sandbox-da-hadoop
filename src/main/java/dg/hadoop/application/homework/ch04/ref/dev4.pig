--读取数据
data =LOAD '/input/lbs.data' USING PigStorage('|') AS (imsi:chararray,time:chararray,loc:chararray);

--注册 jars
REGISTER /opt/hadoop/pig-0.16.0/lib/piggybank.jar;
REGISTER /opt/hadoop/pig-0.16.0/lib/joda-time-2.9.3.jar;
REGISTER /opt/hadoop/pig-0.16.0/lib/datafu-1.2.0.jar;
...
total_count = FOREACH ( GROUP flt BY (time,next_time,loc) ) GENERATE FLATTEN(group) AS (time,next_time,loc), COUNT(flt) AS total;
--计算每一对位置的总数
--pairs_count =FOREACH (GROUP flt BY (loc,next_loc)) GENERATE FLATTEN(group) AS (loc,next_loc),COUNT(flt) AS cnt;
pairs_count = FOREACH ( GROUP flt BY (time,next_time,loc, next_loc) ) GENERATE FLATTEN(group) AS (time,next_time,loc, next_loc), COUNT(flt) AS cnt;
--表链接
...
STORE top3 INTO 'output/hadoop_hw04';
